...
const TOOLS = [
  { name: "get_ngx_market_overview", description: "Get a real-time overview of the Nigerian Stock Exchange (NGX). Returns the All Share Index (ASI), market capitalisation, trading volume, deals, advancers, and decliners. Use this when the user asks about the Nigerian stock market at a high level.", inputSchema: { type: "object", properties: {}, required: [] } },
  { name: "get_ngx_stock_price", description: "Get the latest price and trading history for a specific NGX-listed stock by ticker symbol (e.g. DANGCEM, GTCO, MTNN, ZENITHBANK, ACCESSCORP). Use this when the user asks about the price of a particular Nigerian stock.", inputSchema: { type: "object", properties: { symbol: { type: "string", description: "NGX ticker symbol (e.g. DANGCEM, GTCO, MTNN). Case-insensitive." } }, required: ["symbol"] } },
  { name: "get_ngx_all_stocks", description: "Get the full list of all 148+ equities listed on the NGX with current prices, daily change percent, volume, market cap, and sector.", inputSchema: { type: "object", properties: {}, required: [] } },
  { name: "get_ngx_top_gainers", description: "Get the top gaining stocks on the NGX today, ranked by percentage price increase. Use this when the user asks which Nigerian stocks are up the most today.", inputSchema: { type: "object", properties: { limit: { type: "number", description: "Number of top gainers to return. Defaults to 10." } }, required: [] } },
  { name: "get_ngx_top_losers", description: "Get the top losing stocks on the NGX today, ranked by percentage price decline. Use this when the user asks which Nigerian stocks are down the most today.", inputSchema: { type: "object", properties: { limit: { type: "number", description: "Number of top losers to return. Defaults to 10." } }, required: [] } },
  { name: "get_ngx_market_status", description: "Check whether the Nigerian Stock Exchange (NGX) is currently open or closed. Returns OPEN, CLOSED, or ENDOFDAY.", inputSchema: { type: "object", properties: {}, required: [] } },
  { name: "get_ngx_disclosures", description: "Get the latest 200 corporate disclosures and regulatory announcements from NGX-listed companies.", inputSchema: { type: "object", properties: {}, required: [] } },
  { name: "get_nasd_stocks", description: "Get all 45 equities on Nigeria's NASD OTC Securities Exchange.", inputSchema: { type: "object", properties: {}, required: [] } },
  { name: "get_african_exchanges", description: "Get a list of all African stock exchanges covered by Mansa API — NGX, GSE, NSE, JSE, BRVM, DSE, LuSE, EGX, CSE, BSE, SEM, ZSE, USE — with index levels, daily change, stocks count, and status.", inputSchema: { type: "object", properties: {}, required: [] } },
  { name: "get_african_exchange", description: "Get detailed data for one specific African exchange by ID — index value, change, trading hours, currency, and status.", inputSchema: { type: "object", properties: { exchange: { type: "string", enum: MANSA_EXCHANGES, description: `Exchange ID. Options: ${MANSA_EXCHANGES.join(", ")}` } }, required: ["exchange"] } },
  { name: "get_african_exchange_stocks", description: "Get stocks on a specific African exchange with prices, change percent, volume, market cap, and sector. Supports filtering and sorting.", inputSchema: { type: "object", properties: { exchange: { type: "string", enum: MANSA_EXCHANGES, description: `Exchange ID. Options: ${MANSA_EXCHANGES.join(", ")}` }, limit: { type: "number", description: "Number of stocks to return. Defaults to 50." }, sector: { type: "string", description: "Filter by sector (optional)." }, sort_by: { type: "string", description: "Sort by: price, change_pct, volume, market_cap (optional)." }, order: { type: "string", enum: ["asc", "desc"], description: "Sort order. Defaults to desc." } }, required: ["exchange"] } },
  { name: "get_african_exchange_movers", description: "Get the top gainers and/or losers on a specific African exchange today.", inputSchema: { type: "object", properties: { exchange: { type: "string", enum: MANSA_EXCHANGES, description: `Exchange ID. Options: ${MANSA_EXCHANGES.join(", ")}` }, limit: { type: "number", description: "Number of movers per category. Defaults to 10." }, type: { type: "string", enum: ["gainers", "losers", "both"], description: "Which movers to return. Defaults to both." } }, required: ["exchange"] } },
  { name: "get_pan_african_movers", description: "Get the biggest stock movers across ALL African exchanges combined — top gainers and losers by % change from every covered market today.", inputSchema: { type: "object", properties: { limit: { type: "number", description: "Number of movers per category. Defaults to 10." } }, required: [] } },
  { name: "get_african_indices", description: "Get all major African market indices in one call — NGX ASI, GSE-CI, NASI, J203, BRVM-CI, LASI and more.", inputSchema: { type: "object", properties: {}, required: [] } },
];
